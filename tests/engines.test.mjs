...
// ── macros engine + markdown import + alignment advice ──
{
  ...
  // ── Weekly Muscle Volume Engine ──
  {
    ...
    ok("volume: exercise mapping list is built only from logged workouts", (() => { const list = listExerciseMappings(data, {}); const names = list.map(x => x.norm).sort(); return list.length === 3 && names.join(",") === "bench press,lateral raise,squat" && list.find(x => x.norm === "bench press").muscle === "midChest"; })(), 1);
    ok("volume: empty data → empty mapping list (no predefined exercises)", listExerciseMappings({ exercise: [] }, {}).length === 0, 1);
